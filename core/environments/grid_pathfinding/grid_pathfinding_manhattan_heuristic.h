#ifndef GRID_PATHFINDING_MANHATTAN_HEURISTIC_H_
#define GRID_PATHFINDING_MANHATTAN_HEURISTIC_H_
...
/**
 * A class that defines the Manhattan heuristic function for a grid pathfinding domain.
 *
 * @class GridPathfindingManhattanHeuristic
 */
class GridPathfindingManhattanHeuristic
          : public NodeEvaluatorWithCache<GridLocation, GridDirection>,
            public SingleGoalStateEvaluator<GridLocation> {
public:
    inline static const std::string CLASS_NAME = "GridPathfindingManhattanHeuristic";  ///< The name of the class. Defines this component's name
    /**
     * Creates a Manhattan distance heuristic based on the given goal state.
     *
     * @param goal_state The goal state.
     */
    explicit GridPathfindingManhattanHeuristic(GridLocation goal_state);

    /**
     * Creates a Manhattan distance heuristic based on the given coordiates for the goal state.
     *
     * @param x_coord The x coordinate of the goal state
     * @param y_coord The y coordinate of the goal state
     */
    explicit GridPathfindingManhattanHeuristic(int x_coord = 0, int y_coord = 0);

    /**
     * Default destructor.
     */
    ~GridPathfindingManhattanHeuristic() override = default;
...
#endif /* GRID_PATHFINDING_MANHATTAN_HEURISTIC_H_ */
